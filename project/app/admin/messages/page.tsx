...
export default function AdminMessagesPage() {
  ...
  async function loadConversations() {
    ...
    if (convs && convs.length > 0) {
      const projectIds = [...new Set(convs.map(c => c.project_id))];
  ...
  async function sendMessage() {
    ...
    const { error } = await supabase.from('messages').insert({
      conversation_id: selected.id, sender_id: profile!.id, content: newMessage,
    });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      <PageHeader title="Messages" description="Monitor and participate in client conversations" />
      ...
      {conversations.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm">Conversations are created when you open the chat tab on a project.</p>
        </Card>
      ) : (
            ...
            <div className="space-y-1">
              {conversations.map(conv => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-colors ${selected?.id === conv.id ? 'bg-primary/10' : 'hover:bg-muted'}`}
                >
            ...
            {selected ? (
              <>
                ...
                <div className="border-t border-border p-3 flex gap-2">
                  ...
                  />
                  <button onClick={sendMessage} className="w-10 h-10 rounded-lg bg-gradient-luxe text-white flex items-center justify-center shrink-0">
